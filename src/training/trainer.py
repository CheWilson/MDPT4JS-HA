...
class Trainer:
    ...
    def _display_episode_stats(self, episode, method, stats, env, show_details=False):
        ...
        print(f" [{method}] | Power: {power_str:>8} | Loss: {loss_str:>8} | Success: {success_str:>6} | Jobs: {stats['completed_jobs']:3d}")
    ...
    def train_method(self, env, method, num_episodes, jobs_per_episode, 
        ...
        # 創建包含 AC 配置的保存路徑
        if save_path:
            ac_save_path = os.path.join(save_path)
            if not os.path.exists(ac_save_path):
                os.makedirs(ac_save_path)
                print(f" 創建模型保存目錄: {ac_save_path}")
        else:
            ac_save_path = None
        ...
        for ep in tqdm(range(num_episodes)):
            ...
            if (ep + 1) % 20 == 0 or ep == num_episodes - 1:
                if ac_save_path:
                    self._save_agents(agents, method, ep + 1, ac_save_path)
    ...
    def _save_agents(self, agents, method, episode, save_path):
        ...
        if len(agents) == 1:
            ...
            if hasattr(agent, 'network'):
                ...
                print(f" 已保存 {method} 網絡模型 (Episode {episode})")
                ...
                print(f" 已保存 {method} 三個網絡模型 (Episode {episode})")
        else:
            # 多個代理 (DQN, AC)
            dc_agent, server_agent, ac_agent = agents
            save_model(dc_agent, server_agent, ac_agent, method, episode, save_path)
            print(f" 已保存 {method} 三層代理模型 (Episode {episode})")