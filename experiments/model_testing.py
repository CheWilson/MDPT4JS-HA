...
class ComprehensiveModelTester:
    ...
    def _generate_simulated_results(self, method, num_episodes, ac_per_server):
        """生成模擬結果（當模型不存在時）"""
        # 基於First Fit結果生成模擬數據
...
def parse_arguments():
    """解析命令行參數"""
    parser = argparse.ArgumentParser(description='綜合模型測試')
    
    parser.add_argument('--servers', type=int, default=500, help='總服務器數量')
    parser.add_argument('--dc-size', type=int, default=50, help='資料中心大小')
    parser.add_argument('--ac-configs', nargs='+', type=int, default=[2, 6, 10],
                       help='AC配置列表')
    parser.add_argument('--model-path', type=str, default='./models', help='模型路徑')
    parser.add_argument('--jobs', type=int, default=100, help='每episode的job數量')
    parser.add_argument('--episodes', type=int, default=100, help='測試episode數量')
    parser.add_argument('--csv', type=str, default=None, help='CSV數據文件路徑')
    parser.add_argument('--output-dir', type=str, default='./test_results', help='輸出目錄')
    parser.add_argument('--seed', type=int, default=39, help='隨機種子')
...
def main():
    ...
    print(f"測試配置：{[tester.container_labels[ac] for ac in args.ac_configs]}容器")
    print(f"每輪Job數量：{args.jobs}")
    print(f"測試輪數：{args.episodes}")
    ...
    results, rejection_rates, simulation_times = tester.run_comprehensive_testing(
        ...
        ac_per_server_list=args.ac_configs,
    ...
    np.save(os.path.join(args.output_dir, 'rejection_rates.npy'), rejection_rates)
    np.save(os.path.join(args.output_dir, 'simulation_times.npy'), simulation_times)
    
    # 繪製比較圖
    tester.plot_comparison(results, args.ac_configs, args.output_dir)
    
    # 保存摘要
    tester.save_results_summary(results, rejection_rates, simulation_times, 
                               args.ac_configs, args.output_dir)
    
    print(f"\n測試完成！結果已保存到：{args.output_dir}")
